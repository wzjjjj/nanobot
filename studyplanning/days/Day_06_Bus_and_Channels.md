...
本阶段聚焦于 **“消息高速公路”**，理解 nanobot 如何处理并发与跨平台。

## 学习目标

1.  **MessageBus**：理解 `asyncio.Queue` 实现的生产者-消费者模型。
2.  **Channels**：掌握如何适配不同的聊天平台（如 Telegram, Discord）。
...
*   **文件**：[nanobot/channels/base.py](file:///d:/编程学习记录/nanobot/nanobot/channels/base.py)
*   **动作**：
    *   查看 `BaseChannel` 类。
    *   观察 `_on_message` 回调（接收外部消息 -> 推送 Inbound）。
    *   观察 `send_message` 抽象方法（接收 Outbound -> 发送外部消息）。
*   **思考**：
    *   不同平台的 API 差异巨大，`BaseChannel` 如何抹平差异？
    *   如果平台推送很快，Agent 处理不过来怎么办？
